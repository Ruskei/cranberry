...
template <int NX, int NY, int NZ> struct Sim {
  ...
  void deposit_currents() {
    ...
    const auto fill_support = [](double current_position, int current_cell,
                                 double previous_position, int previous_cell,
                                 int begin, int count, auto &current_weights,
                                 auto &previous_weights, auto &prefix_weights) {
      auto fill_weights = [begin, count](double position, int cell,
        ...
        const std::array<double, 4> local_weights{
            ...
            (-3.0 * fraction_cu + 3.0 * fraction_sq + 3.0 * fraction + 1.0) /
                6.0,
            fraction_cu / 6.0,
        };
    ...
    for (const auto &p : particles) {
      ...
      assert(std::abs(i - pi) <= 1);
      assert(std::abs(j - pj) <= 1);
      assert(std::abs(k - pk) <= 1);
      ...
      for (int y_offset = 0; y_offset < y_count; ++y_offset)
        for (int z_offset = 0; z_offset < z_count; ++z_offset)
          yz_mix[y_offset * max_points + z_offset] =
              mix_pair(y1_form_factors[y_offset], y2_form_factors[y_offset],
                       z1_form_factors[z_offset], z2_form_factors[z_offset]);

      for (int x_offset = 0; x_offset < x_count; ++x_offset)
        for (int z_offset = 0; z_offset < z_count; ++z_offset)
          xz_mix[x_offset * max_points + z_offset] =
              mix_pair(x1_form_factors[x_offset], x2_form_factors[x_offset],
                       z1_form_factors[z_offset], z2_form_factors[z_offset]);

      for (int x_offset = 0; x_offset < x_count; ++x_offset)
        for (int y_offset = 0; y_offset < y_count; ++y_offset)
          xy_mix[x_offset * max_points + y_offset] =
              mix_pair(x1_form_factors[x_offset], x2_form_factors[x_offset],
